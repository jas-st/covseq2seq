...
def get_interaction_dict():
    ...
    for prog, val1 in enumerate(keys):
        ...
        for val2_n in range(prog+1, len(keys)):
            ...
            result = [x for x in set_val1.intersection(set_val2) if "X" not in x]
            if len(result) != 0:
                interaction_dict[frozenset([val1, val2])] = (len(result), result)
                print(val1, val2, result)

    with open("interaction_dict_v2.pkl", "wb") as file:
...
# #
# with open("./interaction_dict_v2.pkl", "rb") as fo:
#     interaction_dict = pickle.load(fo)
# #
# values = [x[0] for x in interaction_dict.values()]
# valuesnp = np.array(values)
# print(valuesnp.mean())
#
# percentile95th = np.percentile(valuesnp, 92)
# print(percentile95th)
#
# for key, value in interaction_dict.items():
#     interaction_dict[key] = value[0]
#     if value[0] > percentile95th:
#         interaction_dict[key] = int(percentile95th)
#
# with open("interaction_dict_normalized.pkl", "wb") as file:
#     pickle.dump(interaction_dict, file, pickle.HIGHEST_PROTOCOL)
